...
On September 12–13, the frozen candidate was attached to MOMENTUM 4 and passed live switch checks and continuous playback with 512 MiB of touched pressure memory. The continuous ETW trace recorded no Glitch events, AudioDG hard faults or lost events. The earlier short-tone trace's seven Glitch events and five host hard faults are also retained in the [hardware record](docs/HARDWARE-2.3.0.md); the overall session is not described as anomaly-free. Sleep action and listening for this binary still await confirmation, so full F03 hardware acceptance remains pending. Windows 10 remains untested. The EXE and ZIP were not rebuilt for this documentation update.
