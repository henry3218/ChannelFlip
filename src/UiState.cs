...
namespace ChannelFlip
{
    public sealed class DevicePreference
    {
        ...
        public OutputDevice Resolve(IList<OutputDevice> active)
        {
            ...
            return new OutputDevice { Id = Id, Guid = guid, Name = String.IsNullOrEmpty(Name) ? "先前選取的裝置" : Name, Offline = true };
        }
    }
    ...
    public sealed class ProcessingObservation
    {
        ...
        public void RecordTest(int channel, bool passed, string failure, DateTime now)
        {
    ...
    public sealed class UiState
    {
        ...
        {
            var result = new UiState { Setting = error != null ? "未知" : !engine.Attached ? "未設定" : engine.Enabled ? "開啟" : "關閉",
                Swap = error == null && engine.Attached && engine.Enabled, Action = engine.Attached ? (engine.Enabled ? "關閉互換" : "開啟互換") : "設定此裝置" };
            if (device == null && error != null) return result.With("read-error", "無法取得裝置狀態", "請重新整理。錯誤詳情可在下方檢視。", true);
            if (device == null) return result.With("empty", "尚無輸出裝置", "請連接耳機或喇叭，再按重新整理。", false);
            if (device.Offline) return result.With("offline", "裝置已離線", "已保留這個裝置。重新連接，或手動選擇其他裝置。", true);
            if (error != null) return result.With("read-error", "無法取得狀態", "請重新整理。錯誤詳情可在下方檢視。", true);
            if (device.Channels < 2 || device.FormatError != null)
                return result.With("unsupported", "目前無法使用左右互換", device.FormatError != null ? "無法取得裝置音訊格式，請檢查連線或 Windows 音效。" : "此裝置目前少於兩個聲道，請切換至立體聲輸出。", true);
            result.CanTest = true;
            ...
            if (!engine.Attached) return result.With("setup", "尚未設定", "首次設定後，即可直接開關互換。也能先測試原始方向。", false);
            if (engine.Error != 0) return result.With("core-error", "核心異常", "可先關閉互換，或到進階設定重新啟動電腦音訊服務。", true);
            if (device.EnhancementsDisabled) return result.With("enhancements-off", "音效強化已關閉", "請在 Windows 音效中開啟此裝置的音效強化，再測試方向。", true);
            if (observation.TestFailure != null) return result.With("test-failed", "本次測試未通過", observation.TestFailure, true);
            if (!engine.Enabled) return result.With("off", "互換已關閉", "設定方向為原本左右；音訊核心與裝置設定仍保留。", false);
            if (observation.Recent(now))
            {
                result.Success = true;
                return result.With("processing", "已偵測到互換處理", observation.HearingConfirmed ?
                    "近期音訊通過互換核心；你已確認的左右方向結果仍保留。" :
                    "近期音訊通過互換核心。請用左右測試確認實際聽到的方向。", false);
            }
            if (observation.LastProcessedUtc != DateTime.MinValue || observation.TestedChannels != 0 || observation.HearingConfirmed)
                return result.With("idle", "目前沒有新音訊", observation.HearingConfirmed ?
                    "互換設定保持開啟；上次左右方向確認仍有效，無須因暫停播放而重新測試。" :
                    "互換設定保持開啟，先前的處理與測試紀錄仍保留；恢復播放後會更新運作狀態。", false);
            return result.With("waiting", "等待音訊／尚待測試", "互換設定已開啟；播放音訊或測試音後才能確認核心處理。", false);
        }
