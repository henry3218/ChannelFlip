...
namespace ChannelFlip
{
    public sealed class MainWindow
    {
        ...
        private readonly ComboBox devices, mode;
        ...
        public MainWindow(AudioSession session, bool simulated)
        {
            ...
            if (simulation) View.Title += " · 模擬";
            UiTheme.Apply(View, null);
            ApplyTextSize();
            SystemParameters.StaticPropertyChanged += OnSystemParametersChanged;
            Microsoft.Win32.SystemEvents.UserPreferenceChanged += OnUserPreferenceChanged;
            View.Activated += delegate { ApplyTextSize(); };
            devices = Get<ComboBox>("Devices"); mode = Get<ComboBox>("SelectionMode");
            ...
            {
                if (simulation) { Session.Message = "模擬模式不會開啟或修改 Windows 音效。"; Update(); return; }
            ...
            Get<Button>("CopyError").Click += delegate
            {
                ...
                catch (Exception ex) { Session.Message = "無法複製：" + ex.Message; Update(); }
            };
            ...
            timer.Tick += delegate
            {
                ...
                if (devices.IsDropDownOpen || mode.IsDropDownOpen) Update();
        ...
        private static AudioSession CreateLiveSession()
        {
            ...
            catch (Exception ex) { preference = new DevicePreference(); error = "無法讀取裝置偏好：" + ex.Message; }
        ...
        private void SavePreference()
        {
            ...
            string key = Session.Preference.FollowDefault + "|" + Session.Preference.Id + "|" + Session.Preference.Name;
            if (preferenceKey == key) return;
            preferenceKey = key;
            try { Session.Preference.Save(Program.DataDirectory); }
            catch (Exception ex) { Session.ErrorDetail = "裝置偏好未能儲存：" + ex.Message; Update(); }
        }
        ...
        public void Update()
        {
            ...
            try
            {
                ...
                if (itemsKey != key) { devices.ItemsSource = Session.Devices; itemsKey = key; }
                ...
                devices.IsEnabled = !busy; mode.IsEnabled = !busy;
                Get<Button>("Refresh").IsEnabled = !busy;
                Get<TextBlock>("DeviceDetail").Text = Session.Preference.FollowDefault ?
                    "操作目標跟隨多媒體預設；各裝置設定獨立保存。" :
                    device == null ? "選擇要操作的耳機或喇叭。" : "固定此裝置；斷線時保留選取。";
                Get<TextBlock>("SettingValue").Text = "設定：" + p.Setting;
                ...
                AutomationProperties.SetHelpText(toggle, Session.ReadError != null ? "目前無法讀取設定；請重新整理。" :
                    device != null && device.Offline ? "設定目前" + p.Setting + "；裝置已離線，重新連接後才能切換。" :
                    p.Swap ? "目前開啟；按空白鍵關閉此裝置的互換。" : "目前關閉；按空白鍵開啟此裝置的互換。");
                Get<TextBlock>("RuntimeTitle").Text = Session.Busy ? "正在處理…" : (p.Warning ? "！ " : p.Success ? "✓ " : "") + p.Title;
                ...
                Get<TextBlock>("RuntimeDetail").Text = Session.Busy ? (Session.Message ?? "請稍候。") : p.Detail;
                bool unknown = device == null || Session.ReadError != null;
                Get<TextBlock>("LeftMapping").Text = unknown ? "來源左 → 待確認" : p.Swap ? "來源左 → 右耳" : "來源左 → 左耳";
                Get<TextBlock>("RightMapping").Text = unknown ? "來源右 → 待確認" : p.Swap ? "來源右 → 左耳" : "來源右 → 右耳";
                ...
                string evidence = observation.HearingConfirmed ? "你已確認左右方向符合（" + observation.TestUtc.ToLocalTime().ToString("HH:mm:ss") + "）。" :
                    observation.TestUtc == DateTime.MinValue ? (p.CanTest ? "左右測試可確認實際聽到的方向。" : "裝置可用後，才能測試左右方向。") :
                    observation.TestFailure != null ? "上次測試未通過 · " + observation.TestUtc.ToLocalTime().ToString("HH:mm:ss") :
                    "上次測試完成 · " + observation.TestUtc.ToLocalTime().ToString("HH:mm:ss") + "；實際方向待你確認。";
                ...
                string detail = String.Join("\n\n", new[] { Session.ReadError, Session.ErrorDetail,
                    Session.State.Error == 0 ? null : "核心錯誤：0x" + Session.State.Error.ToString("X8"), Session.ScopeError == null ? null : "全域設定：" + Session.ScopeError }.Where(s => !String.IsNullOrEmpty(s)));
        ...
        private async System.Threading.Tasks.Task Setup()
        {
            ...
            string text = "設定裝置：" + target.Name + "\n\n本程式會備份此裝置的音效設定、接入內建音訊核心，並開啟左右互換。\n\n電腦音訊服務會重新啟動，所有裝置的聲音會短暫中斷；完成時會播放兩聲短音檢查核心。\n\n";
            text += needs ? "此預覽版核心尚未取得 Microsoft 音訊簽章。首次設定需調整受保護音訊宿主設定（DisableProtectedAudioDG=1），作用於整台電腦，部分 DRM 音訊可能受影響。\n\n" :
                "此電腦目前已允許載入此音訊核心；本次會沿用現有的音訊宿主設定。\n\n";
            text += "關閉互換或視窗會保留系統設定。日後可在進階設定使用「移除所有裝置的設定」還原本程式的變更。";
            if (Dialog("設定此裝置", text, "設定並開啟互換") != "accept") return;
        ...
        private async System.Threading.Tasks.Task Advanced()
        {
            ...
            Session.Refresh();
            if (Session.ScopeError != null) { Dialog("無法讀取進階設定", Session.ScopeError, null); return; }
            var scope = Session.Scope;
            var active = Session.Devices.Where(d => !d.Offline).Select(d => d.Guid).ToArray();
            string names = String.Join("\n", scope.Devices.Select(d => "• " + d.Name + (active.Contains(d.Id, StringComparer.OrdinalIgnoreCase) ? "" : "（目前離線）")));
            string text = "本程式已設定 " + scope.Devices.Length + " 個裝置。\n" + names +
                "\n\n關閉所有互換：只恢復這些裝置的左右方向，保留核心與系統設定。\n\n移除所有裝置的設定：還原本程式管理的裝置音效、核心登記及音訊宿主設定，包含離線裝置。外部程式已修改的設定會保留。\n\n重新啟動或移除時，整台電腦的音訊會短暫中斷。";
            string choice = Dialog("進階設定", text, null, scope);
            if (choice == null) return;
            string confirmation = choice == "remove" ? "將移除上述所有裝置的設定，並還原本程式管理的系統變更。" :
                choice == "off" ? "將關閉上述所有裝置的互換，保留核心與系統設定。" : "將重新啟動整台電腦的音訊服務。";
            if (choice != "off") confirmation += "\n\n所有音訊輸出的聲音會短暫中斷。";
            if (Dialog(choice == "remove" ? "移除所有裝置的設定" : choice == "off" ? "關閉所有裝置的互換" : "重新啟動電腦音訊服務",
                confirmation + "\n\n影響的已設定裝置：\n" + (names.Length == 0 ? "沒有裝置；仍可能有全域設定。" : names),
                choice == "remove" ? "還原並移除設定" : "確認執行") != "accept") return;
        ...
        public string Dialog(string title, string text, string accept, SetupScope scope = null)
        {
            ...
            var cancel = new Button { Content = accept == null && scope == null ? "關閉" : "取消", IsCancel = true, Margin = new Thickness(6, 0, 0, 0) };
            ...
            if (scope != null)
            {
                ...
                string[] labels = { "關閉所有已設定裝置的互換", "重新啟動電腦音訊服務", "移除所有裝置的設定" };
        ...
        {
            if (!simulation) throw new InvalidOperationException("離線預覽只能使用模擬資料。");
            ...
            var frame = new Border { Background = View.Background, Child = content, Width = width, Height = height, Resources = View.Resources };
