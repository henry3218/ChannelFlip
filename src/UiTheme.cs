...
namespace ChannelFlip
{
    ...
    public static class Scenarios
    {
        ...
        public static AudioSession Create(string name)
        {
            ...
            var device = new OutputDevice { Id = "{0.0.0.00000000}." + guid, Guid = guid, Name = "耳機 (MOMENTUM 4)", Channels = 2, IsDefault = true };
            ...
            if (state.Attached) backend.Setup = new SetupScope { Signature = "simulation", CoreRegistered = true, HostSettings = true,
                Devices = new[] { new SetupDevice { Id = guid, Name = device.Name }, new SetupDevice { Id = "{22222222-2222-2222-2222-222222222222}", Name = "USB 喇叭" } } };
            ...
            if (name == "read-error") backend.ReadFailure = "模擬：無法讀取音訊核心狀態檔。";
            ...
            if (name == "long-name") device.Name = "客廳電腦的無線降噪耳機 — 很長的裝置名稱，用來確認完整文字仍然可以閱讀 (MOMENTUM 4 Wireless)";
            ...
            if (name == "test-failed") session.Observation.RecordTest(0, false, "測試期間未偵測到預期處理，請重試或檢查 Windows 音效設定。", DateTime.UtcNow);
            if (name == "busy") { session.Busy = true; session.Message = "正在設定此裝置，電腦音訊會短暫中斷…"; }
            return session;
        }
    }
}
